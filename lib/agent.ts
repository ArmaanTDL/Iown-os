...
export async function runAgent(userId: string) {
    ...
    try {
        ...
        const results: PromiseSettledResult<{
            ...
            actionItems?: { title: string; description: string; dueDate: string | null }[];
            calendarEvents?: { title: string; description: string; date: string; startTime: string | null; endTime: string | null }[];
            tasksCreated?: number;
        ...
        // Process emails sequentially to avoid Gemini free-tier rate limits
        for (const email of emails) {
                        ...
                        if (calendarClient && analysis.calendarEvents?.length > 0) {
                            for (const event of analysis.calendarEvents) {
                                try {
                                    ...
                                    console.error(`[Agent] Failed to create calendar event "${event.title}":`, err);
                                }
                            }
                        }
            ...
            results.push(...result);
            // Wait 2 seconds between emails to respect Gemini free-tier rate limits
